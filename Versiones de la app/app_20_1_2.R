...
# ══════════════════════════════════════════════════════════════════════════════
# CAMBIOS SESIÓN 6 (sobre la base de app_20.R cerrada en Sesión 5):
#
#   1. Navbar sticky (position:sticky en .navbar)
#   2. Canvas más ancho: tab_wrap 680px -> 960px; prosa larga (.exp-p, .ant-p,
#      .sec-desc, .conclu-p, .caso-resumen, .meto-p, .enc-bajada,
#      .eje-cierre-txt, .eje-cierre-nota, .ciclo-footer) a 820px de tope
#   3. Eje 1 y Eje 2: dimensiones convertidas a selector de tarjetas arriba +
#      panel único abajo, con fade al cambiar (Plotly.Plots.resize() +
#      suspendWhenHidden = FALSE en los outputs correspondientes). Mecanismo
#      viejo eje1_rv/eje2_rv (ya muerto desde antes de esta sesión) retirado.
#   4. Fix: graf_costos y graf_barras_h mostraban "tipo"/"valor"/"costo" como
#      título de eje (nombre de columna por defecto de Plotly) — se agregó
#      title = "" explícito en ambas funciones, y defensivamente en
#      graf_progreso.
#   5. Eje 2 Dim 2 (Salud): de impact-grid a metrics-row + graf_barras_h con
#      las dos cifras reales (ambulancia 18,75% / amenazas físicas 91,3%).
#      No se fabricó un "promedio nacional" para esa dimensión: no existe
#      dato verificado de comparación nacional para ambulancia.
#   6. Botón "Leer más" (.exp-btn): de outline gris transparente a fondo
#      violeta sólido, para que se note más.
#   7. Ciclo de Eje 2: título "El ciclo" -> "Barrera tras barrera"; pie
#      actualizado.
#   8. Conclusión de "Qué se sabe": ancho de párrafo alineado con el resto.
# ══════════════════════════════════════════════════════════════════════════════
...
casos <- list(
  list(
    ...
    documento = list(
      txt = "AEI — Does Social Housing Actually Work? Setting the Record Straight on the Vienna Model (2023)",
  ...
  list(
    ...
    documento = list(
      txt = "IZA Policy Paper 203 — The Global Housing Affordability Crisis: Policy Options (Floetotto, Nguyen, Sieg, 2021)",
  ...
  list(
    ...
    documento = list(
      txt = "FEANTSA — Housing First Europe: evidencia y resultados",
...
.exp-p,.ant-p,.sec-desc,.conclu-p,.caso-resumen,.meto-p{max-width:820px;}
...
.punto-li::before{content:"\2022";color:#AFA9EC;font-size:14px;line-height:1.7;flex-shrink:0;}
    ...
    div(class = "dim-panel-area", id = "e1_panel_area",
      ...
      div(id = "e1_panel_1", class = "dim-panel",
        ...
        div(id = "ep_e1d1", class = "exp-panel",
          ...
          p(class = "exp-p", "En doce años, ese presupuesto se multiplicó por diez. En 2023 llegó a $45.071 millones: más del doble que cuatro años antes. En el mismo período, el número de campamentos en Chile pasó de 802 a 1.428, y el de familias de 47.050 a 120.584."),
          p(class = "exp-p", "Esto no es un argumento contra el programa. Es un argumento contra la idea de que el programa solo puede dar más de lo mismo. El gasto sistemático sin solución de fondo no resuelve la crisis. Solo paga el costo de que siga existiendo."),
      ...
      div(id = "e1_panel_2", class = "dim-panel",
        ...
        div(id = "ep_e1d2b", class = "exp-panel",
          ...
          p(class = "exp-p", "Como referencia del costo global del programa: dividiendo el presupuesto 2023 ($45.071 millones) por los hogares atendidos en habitabilidad primaria ese año, el costo promedio por familia en el sistema de espera ronda los $9.870.000.")
        )
      ),
      ...
      div(id = "e1_panel_3", class = "dim-panel",
        div(class = "metrics-row",
          ...
          div(class = "metric-card", div(class = "met-label", "Avance a mayo 2025"), div(class = "met-val nar", "27%"), div(class = "met-sub", "de ese compromiso")),
        ...
        p(class = "graf-note", "TECHO-Chile, Segundo Reporte de avances PEH en campamentos (sept. 2025), con datos MINVU a mayo 2025."),
        ...
        div(id = "ep_e1d3", class = "exp-panel",
          p(class = "exp-p", "Chile no tiene una política de Estado para la vivienda. Tiene planes de gobierno que se renuevan cada cuatro años. El más ambicioso hasta ahora fue el Plan de Emergencia Habitacional (PEH), lanzado en 2022: 260.000 viviendas comprometidas, cumplidas en marzo de 2026."),
          p(class = "exp-p", "El PEH no construye viviendas directamente en la mayoría de los casos. Opera principalmente a través de subsidios habitacionales: el DS49 (fondo solidario, para los hogares más vulnerables) y el DS19 (integración social, para proyectos con mezcla de ingresos) concentran la mayor parte de las viviendas entregadas. El Estado financia la demanda, pero son inmobiliarias y constructoras, públicas o privadas, las que ejecutan la oferta."),
          p(class = "exp-p", "Dentro de ese compromiso había uno específico para campamentos: atender a 76.004 hogares mediante alguna solución habitacional. Cuando se diseñó el plan, había alrededor de 80.000 familias en esa condición, es decir, el compromiso apenas las cubría. A mayo de 2025, el avance era del 27%."),
          p(class = "exp-p", "En ese mismo período, el número de hogares en campamentos subió a 120.584. El plan más ambicioso en décadas fue diseñado para una crisis que ya era distinta —y más grande— al momento de ejecutarse."),
