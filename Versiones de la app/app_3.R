  ...
  tags$script(HTML("
    function toggleEje(id, btn, ejeKey) {
      ...
      if (open) {
        ...
        // Paso 2: tras 350ms (DOM ya pintado), fuerza resize en cada
        // grafico Plotly del contenedor para que recalcule dimensiones
        setTimeout(function() {
...
server <- function(input, output, session) {
  ...
  output$graf_e1d1 <- renderPlotly({
    eje1_rv()  # dependencia reactiva que fuerza re-render al abrir
