...
/* Fuerza altura en contenedores de Plotly para evitar colapso a cero
   cuando el padre tiene display:none al momento del primer render */
.plotly-output-container, .shiny-plot-output, .shiny-html-output { min-height: 10px; }
...
ui <- fluidPage(
  ...
  div(class = "pagina",
    ...
    div(class = "sec-wrap",
      ...
      acord("meto", "¿Cómo se construyó esta aplicación?",
        div(
          ...
          p(class = "meto-p", "Todo dato tiene fuente explícita y URL verificable en la sección de fuentes al final de la página."),
          p(class = "meto-p", "Los datos de TECHO-Chile se presentan con disclaimer cuando corresponde: son representativos de campamentos catastrados, no del universo total de informalidad habitacional."),
          p(class = "meto-p", "Los estimados de DIPRES se presentan como tales — no como costos ejecutados ni auditados."),
          p(class = "meto-p", "Cuando dos fuentes corresponden a años distintos, se declara explícitamente. No se usan datos con más de cinco años de antigüedad sin nota aclaratoria.")
        )
      )
    ),
    ...
    div(class = "sec-wrap",
      ...
      div(class = "eje-card",
        ...
        tags$button(class = "eje-abrir-btn",
          onclick = "toggleEje('eje1_contenido', this, 'eje1')",
          "↓ Ver Eje 1")
      ),
      ...
      div(id = "eje1_contenido", class = "eje-contenido",
        ...
        div(class = "dim-block",
          ...
          graf_note("INDH Informe Anual 2024 / DIPRES / TECHO-Chile Catastros 2011–2025."),
          expand_block("e1d1",
        ...
        div(class = "dim-block",
          ...
          graf_note("INDH Informe Anual 2024 / TECHO-Chile Catastro 2024–2025."),
          expand_block("e1d2",
        ...
        div(class = "dim-block",
          ...
          graf_note("DIPRES Evaluación ex ante 2025. Estimados presupuestarios — no costos ejecutados auditados."),
          expand_block("e1d3",
      ...
      div(class = "eje-card", style = "margin-top:1rem;",
        ...
        tags$button(class = "eje-abrir-btn",
          onclick = "toggleEje('eje2_contenido', this, 'eje2')",
          "↓ Ver Eje 2")
      ),
      ...
      div(id = "eje2_contenido", class = "eje-contenido",
        ...
        div(class = "dim-block",
          ...
          graf_note("TECHO-Chile Mapa del Derecho a la Ciudad 2023 / INE ENE oct–dic 2025. Dato TECHO: campamentos catastrados."),
          expand_block("e2d1",
        ...
        div(class = "dim-block",
          ...
          graf_note("TECHO-Chile 2023 / INDH 2024. Distancias a salud primaria: solo Región Metropolitana."),
          expand_block("e2d2",
        ...
        div(class = "dim-block",
          ...
          graf_note("Fundación Recrea 2025, 5 regiones (RM, Valparaíso, Antofagasta, Biobío, Tarapacá)."),
          expand_block("e2d3",
        ...
        div(class = "dim-block",
          ...
          graf_note("Fundación Recrea 2025 / CASEN 2022. Años distintos — declarado explícitamente."),
          expand_block("e2d4",
        ...
        div(class = "dim-block",
          ...
          graf_note("TECHO-Chile Catastro 2024–2025 / SciELO (Contreras et al.) 2022 / INDH 2024."),
          expand_block("e2d5",
    ...
    div(class = "sec-wrap",
      ...
      p(class = "qss-intro",
        "La literatura internacional sobre crisis habitacional identifica intervenciones que han reducido ",
        "el déficit en distintos contextos. Ninguna es directamente replicable: cada caso emergió de ",
        "condiciones históricas y de control estatal del suelo que no son universales. ",
        "Lo que sí es transferible son los principios que subyacen a cada modelo."
      ),

      lapply(casos, function(c) {
        div(class = "caso-card",
          div(class = "caso-header",
            div(class = "caso-icono", c$icono),
            div(
              p(class = "caso-pais", c$pais),
              p(class = "caso-titulo", c$titulo)
            )
          ),
          p(class = "caso-resumen", c$resumen),
          expand_block(paste0("caso_", gsub("[^a-z]", "", tolower(c$pais))),
    ...
    div(class = "footer-wrap",
      ...
      p(class = "footer-ga",
        "Esta aplicación usa Google Analytics para medir visitas de forma anónima."
      )
    )
  ...
  # ══════════════════════════════════════════════════════════════════════════
  # JAVASCRIPT
  # Fix de Plotly en contenedores ocultos:
  # toggleEje notifica a Shiny qué eje se abrió → reactiveVal se incrementa
  # → renderPlotly se re-ejecuta con el contenedor ya visible
  # ══════════════════════════════════════════════════════════════════════════
  tags$script(HTML("
    function toggleEje(id, btn, ejeKey) {
      var c = document.getElementById(id);
      var open = c.classList.toggle('abierto');
      var label = ejeKey === 'eje1' ? 'Eje 1' : 'Eje 2';
      btn.textContent = open ? '↑ Cerrar' : '↓ Ver ' + label;
      if (open) {
        // Paso 1: notifica a Shiny para re-ejecutar renderPlotly
        Shiny.setInputValue(ejeKey + '_opened', Math.random(), {priority: 'event'});
        // Paso 2: tras renderizar (req > 0 dispara el render en Shiny),
        // fuerza resize en dos momentos para cubrir distintas velocidades de red
        function resizePlots() {
          var plots = document.getElementById(id).querySelectorAll('.js-plotly-plot');
          plots.forEach(function(plot) {
            try { Plotly.relayout(plot, {autosize: true}); } catch(e) {}
          });
        }
        setTimeout(resizePlots, 600);
        setTimeout(resizePlots, 1400);
      }
    }
