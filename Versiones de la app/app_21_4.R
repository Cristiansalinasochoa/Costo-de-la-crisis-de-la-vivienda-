...
# PÁRRAFOS DE ENTRADA
intro_parrafos <- list(
  HTML("En Chile, acceder a una vivienda digna es cada vez más difícil. La tasa de propiedad cayó diez puntos en veinte años. Uno de cada cuatro hogares arrienda hoy, y al menos uno de cada siete destina más del 30% de sus ingresos a ese arriendo, el umbral que la ONU define como insostenible. <strong>Más de 100.000 familias viven en casa de otra familia</strong> porque no tienen dónde más ir. Y quienes tienen vivienda propia no están necesariamente a salvo: uno de cada cinco hogares en Chile presenta alguna carencia habitacional, aunque sea propietario."),
...
cierre_antecedentes <- HTML("La crisis habitacional no empieza en el campamento. <strong>El campamento es el punto de llegada de una cadena de exclusiones</strong> que puede comenzar en el arriendo, en el allegamiento o en una vivienda que no cumple condiciones mínimas.")

# ══════════════════════════════════════════════════════════════════════════════
# DATOS — MAPA "¿DÓNDE ESTÁN LOS CAMPAMENTOS?" (ANTECEDENTES)
# ══════════════════════════════════════════════════════════════════════════════
# data/comunas_chile.geojson: 346 comunas, geometría oficial (shapefile
#   MINVU/BCN, reproyectado de Web Mercator a WGS84 y simplificado).
# data/campamentos_por_comuna.csv: agregado desde Nomina-Campamentos-Catastro-
#   2026.xlsx (MINVU, Catastro vigente), con codigo_comuna ya resuelto contra
#   el shapefile oficial — 191/191 comunas cruzan, verificado.
#
# El geojson con estilo y popup inyectado se arma UNA SOLA VEZ al arrancar la
# app (acá, a nivel global — no dentro de renderLeaflet), para no repetir el
# trabajo de las 346 features en cada sesión de usuario.
#
# NOTA IMPORTANTE — cifras de este mapa vs. el resto de la app:
# El total de este catastro (1.345 campamentos, 84.252 hogares) es menor al
# de TECHO-Chile (1.428 campamentos, 120.584 familias) usado en el resto de
# la app. Es una discrepancia institucional real y documentada ante el
# Congreso (ver nota_mapa_campamentos abajo) — no un error de datos.

geo_campamentos   <- jsonlite::fromJSON("data/comunas_chile.geojson", simplifyVector = FALSE)
datos_campamentos <- read.csv("data/campamentos_por_comuna.csv", encoding = "UTF-8")

hogares_lookup_comuna     <- setNames(datos_campamentos$hogares, as.character(datos_campamentos$codigo_comuna))
campamentos_lookup_comuna <- setNames(datos_campamentos$campamentos, as.character(datos_campamentos$codigo_comuna))

pal_campamentos <- colorBin(
  palette  = c("#F4F1FB", "#D9CFF0", "#B6A3E6", "#7F77DD", "#D85A30"),
  domain   = c(0, datos_campamentos$hogares),
  bins     = c(0, 1, 100, 500, 1500, Inf),
  na.color = "#F4F1FB"
)

for (i in seq_along(geo_campamentos$features)) {
  cod  <- as.character(geo_campamentos$features[[i]]$properties$codigo_comuna)
  hog  <- unname(hogares_lookup_comuna[cod]);     if (is.na(hog))  hog  <- 0
  camp <- unname(campamentos_lookup_comuna[cod]); if (is.na(camp)) camp <- 0
  nombre <- geo_campamentos$features[[i]]$properties$nombre_comuna
  
  geo_campamentos$features[[i]]$properties$style <- list(
    fillColor   = pal_campamentos(hog),
    fillOpacity = 0.85,
    color       = "#FFFFFF",
    weight      = 0.4
  )
  geo_campamentos$features[[i]]$properties$popup <- if (camp > 0) {
    sprintf("<strong>%s</strong><br>%d campamentos<br>%s hogares",
            nombre, camp, format(hog, big.mark = "."))
  } else {
    sprintf("<strong>%s</strong><br>Sin campamentos registrados", nombre)
  }
}

geojson_campamentos_final <- jsonlite::toJSON(geo_campamentos, auto_unbox = TRUE)

nota_mapa_campamentos <- paste(
  "Este mapa usa el Catastro Nacional de Campamentos vigente del MINVU (2026).",
  "Su cifra de hogares es menor a los 120.584 de TECHO-Chile citados en el resto",
  "de esta aplicación. La diferencia no es un error: el propio MINVU ha explicado",
  "ante el Congreso que responde a metodologías y periodos de levantamiento",
  "distintos, y a criterios distintos para definir qué es un campamento. Que el",
  "Estado y la sociedad civil midan el problema de forma distinta es, en sí",
  "mismo, parte de lo que esta aplicación documenta."
)

nota_actualizacion_mapa <- paste(
  "El catastro geoespacial del MINVU fue actualizado en febrero de 2026",
  "(Res. Ex. N° 377), consolidando los registros de 2011, 2019, 2022 y 2024."
)
...
casos <- list(
  list(
    ...
    leer_mas = list(
      ...
      HTML("El sistema no se diseñó solo para los hogares más pobres. <strong>El 75% de la población califica para acceder a vivienda pública o cooperativa</strong>, lo que mezcla distintos niveles de ingreso en los mismos edificios y barrios. No hay guetos de vivienda social porque casi cualquiera puede vivir en ella."),
      HTML("Los arriendos subsidiados se fijan por décadas y no responden a los ciclos del mercado inmobiliario. <strong>Esa estabilidad reduce la presión sobre el arriendo privado</strong>, que en Chile es la puerta de entrada más común hacia un campamento."),
      HTML("El modelo surgió de una condición que Chile no tiene: control municipal extenso del suelo urbano, sostenido durante cien años. No es un programa trasladable tal cual. <strong>Lo que sí ilustra es qué pasa cuando el Estado deja de tratar la vivienda exclusivamente como un bien de mercado</strong>.")
    ),
    documento = list(
  ...
  list(
    ...
    leer_mas = list(
      HTML("<strong>El gobierno de Singapur controla alrededor del 90% del suelo del país</strong> y lo usa para subsidiar la construcción y venta de vivienda pública a gran escala. El programa nació en los años sesenta como respuesta a una crisis de vivienda informal de magnitud comparable, en proporción a la población, a la que enfrenta Chile hoy."),
      HTML("A diferencia de Viena, el modelo no se basa en arriendo sino en propiedad. Las familias compran su unidad con subsidio estatal y pueden venderla más adelante en el mercado secundario. <strong>El 82% de los residentes vive en vivienda construida por el Estado</strong>, lo que convierte esta modalidad en la opción por defecto."),
      HTML("Replicar este modelo requiere algo que la mayoría de los países, Chile incluido, no tiene: <strong>control estatal del suelo desde el origen del proceso de urbanización</strong>. Donde el suelo ya está privatizado y fragmentado, como ocurre en las ciudades chilenas, implementar un modelo equivalente es extremadamente costoso."),
      HTML("Lo que Chile puede tomar de este caso es la condición que la hizo posible: <strong>cuando el Estado interviene el mercado del suelo de forma sostenida</strong>, la vivienda pública deja de ser la excepción.")
    ),
    documento = list(
  ...
  list(
    ...
    leer_mas = list(
      HTML("<strong>Finlandia redujo la población en situación de calle en más de un 70% en treinta años</strong> con un cambio de principio: dejó de exigir que las personas resolvieran otros problemas (adicciones, salud mental, empleo) antes de acceder a una vivienda. La vivienda estable pasó a ser la base del proceso, y dejó de ser vista como un premio a ganar después del martirio."),
      ...
      HTML("La diferencia de escala importa. Finlandia diseñó este sistema para una población sin hogar de algunas decenas de miles de personas. En Chile, los campamentos concentran a <strong>120.584 familias</strong>. El principio es transferible. Pero la maquinaria institucional necesaria para mantenerlo, son palabras mayores.")
    ),
    documento = list(
  ...
  list(
    ...
    leer_mas = list(
      HTML("Entre 2009 y 2018, <strong>Brasil entregó más de cinco millones de viviendas</strong> mediante subsidios directos a familias de bajos ingresos, con participación del sector privado como constructor. Es el programa de vivienda social más grande de América Latina, y en menos de una década logró reducir de forma significativa el déficit habitacional cuantitativo del país."),
      HTML("El problema apareció en la localización. Para construir a esa velocidad y a ese costo, gran parte de las viviendas se levantaron donde el suelo era más barato: la periferia urbana, lejos del trabajo, el transporte y los servicios. <strong>El déficit cuantitativo bajó. Pero eso no disminuyó la segregación urbana</strong>."),
      HTML("La lección para Chile es directa, porque el mecanismo es el mismo que documenta el Eje 1 de este proyecto: <strong>un subsidio sin control de localización no resuelve la exclusión</strong>. La traslada y la formaliza. La vivienda deja de ser informal, pero sigue lejos de donde está la ciudad.")
    ),
    documento = list(
...
.mapa-camp-bloque{margin-top:2.5rem;}
...
.sim-comparacion{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:1rem 0;}
.sim-columna{background:#f4f2fc;border-radius:7px;padding:.85rem .9rem;text-align:center;}
.sim-columna-acento{background:#fbeae5;}
.sim-col-label{font-size:11.5px;color:#888780;margin:0 0 5px;}
.sim-col-sub{font-size:11px;color:#999;margin:5px 0 0;}
.sim-card-val-acento{color:#D85A30;}
  ...
  tab_wrap(
    ...
    div(class = "sec-wrap",
        ...
        div(style = "margin-top:1.25rem;",
            ...
            p(class = "ant-p",
              tags$strong("Hoy existen 1.428 campamentos"), " en el país, donde viven 120.584 familias: cerca de 400.000 personas. ",
    ...
    div(class = "sec-wrap",
        ...
        div(style = "margin-top:1.25rem;",
            ...
            p(class = "ant-p",
              "El objetivo es ", tags$strong("reenfocar la discusión pública"), " sobre la vivienda, y en particular sobre los campamentos, ",
              "hacia el problema central: cómo facilitar el acceso a una vivienda digna para las personas en Chile."
            ),
            p(class = "ant-p",
              tags$strong("Porque las personas en campamentos no son quienes producen la crisis"), ", sino su síntoma más grave. ",
...
tab_antecedentes <- bslib::nav_panel(
  ...
  tab_wrap(
    ...
    div(class = "mapa-camp-bloque",
        h3(class = "inv-titulo", "¿Dónde están los campamentos?"),
        p(class = "ant-p", "Los campamentos no se concentran en un solo lugar. Están en 191 comunas, en las 16 regiones del país."),
        leafletOutput("mapa_campamentos", height = "420px"),
        p(class = "nota-met", nota_mapa_campamentos),
        p(class = "nota-met", nota_actualizacion_mapa)
    ),
...
tab_eje1 <- bslib::nav_panel(
  ...
  tab_wrap(
    ...
    p(class = "sec-desc",
      ...
      "El presupuesto se cuadruplicó en cinco años, pero eso no impidió que el número de campamentos también aumentara."
    ),
    ...
    div(class = "dim-panel-area", id = "e1_panel_area",
        ...
        div(id = "e1_panel_1", class = "dim-panel",
            ...
            div(id = "ep_e1d1", class = "exp-panel",
                p(class = "exp-p", "El Estado chileno tiene un programa específico para intervenir en campamentos: el Programa de Asentamientos Precarios, que depende del MINVU y existe desde 2011. Cuando nació, tenía un presupuesto de ", tags$strong("$4.461 millones anuales"), " y operaba exclusivamente a través de municipalidades."),
                p(class = "exp-p", "En doce años, ese presupuesto ", tags$strong("se multiplicó por diez"), ". En 2023 llegó a $45.071 millones: más del doble que cuatro años antes. En el mismo período, el número de campamentos en Chile pasó de 706 a 1.428, y el de familias de 47.050 a 120.584."),
                p(class = "exp-p", "Esto no es un argumento contra el programa ni implica que sus intervenciones individuales sean ineficaces. Implica que ", tags$strong("el flujo de entrada de nuevas familias a la informalidad supera la capacidad de cierre del sistema"), ". El gasto sistemático sin solución de fondo no resuelve la crisis. Solo paga el costo de que siga existiendo."),
        ...
        div(id = "e1_panel_2", class = "dim-panel",
            ...
            div(id = "ep_e1d2b", class = "exp-panel",
                ...
                p(class = "exp-p", "Según estimaciones presupuestarias de DIPRES 2025, una solución transitoria —una vivienda temporal mientras se gestiona la definitiva— cuesta alrededor de $3.316.000 por hogar. La urbanización, que significa dotar al campamento de servicios básicos y regularizar su tenencia, cuesta aproximadamente ", tags$strong("$7.194.000 por hogar"), "."),
                p(class = "exp-p", tags$strong("Ninguna de estas cifras incluye el subsidio habitacional final"), " que las familias necesitan para escriturar una vivienda propia. Ese costo viene después, y es adicional."),
                p(class = "exp-p", "Como referencia del costo global del programa: dividiendo el presupuesto 2023 ($45.071 millones) por los 4.565 hogares beneficiados en habitabilidad primaria durante 2022 (el año más reciente con ese nivel de desglose disponible), el costo promedio por familia en el sistema de espera ronda los ", tags$strong("$9.870.000"), ". Es una estimación propia que cruza dos años distintos por falta de un dato más actualizado, y no refleja el costo de todos los componentes del programa.")
            )
        ),
        ...
        div(id = "e1_panel_3", class = "dim-panel",
            ...
            div(id = "ep_e1d3", class = "exp-panel",
                p(class = "exp-p", "Chile no tiene una política de Estado para la vivienda. Tiene planes de gobierno que se renuevan cada cuatro años. El más ambicioso hasta ahora fue el Plan de Emergencia Habitacional (PEH), lanzado en 2022, con la meta nacional de ", tags$strong("260.000 viviendas"), ". El Gobierno anunció el cumplimiento de esa meta nacional en marzo de 2026."),
                p(class = "exp-p", "Esa meta nacional no se reparte de forma pareja. Dentro del plan había un compromiso específico para campamentos: atender a ", tags$strong("76.004 hogares"), " mediante alguna solución habitacional. Cuando se diseñó el plan, había alrededor de 80.000 familias en esa condición, es decir, el compromiso apenas las cubría."),
                p(class = "exp-p", "Según TECHO-Chile, a mayo de 2025 el avance específico en campamentos era de solo ", tags$strong("27%"), ", muy por debajo del cumplimiento de la meta nacional general. El Ministerio de Vivienda (MINVU) ha disputado esta cifra públicamente, señalando un avance de 45% sobre una meta base distinta (25.894 hogares en vez de 76.004). La diferencia refleja un desacuerdo sobre qué programas y qué universo de hogares cuenta como \"intervención en campamentos\", no una corrección de un error de cálculo."),
                p(class = "exp-p", "Bajo cualquiera de las dos lecturas, ", tags$strong("el patrón se mantiene: la meta nacional general avanzó mucho más rápido que la meta específica para campamentos"), ". En ese mismo período, el número de hogares en campamentos subió a 120.584. El plan más ambicioso en décadas fue diseñado para una crisis que ya era distinta —y más grande— al momento de ejecutarse."),
                p(class = "exp-p", tags$strong("Esto demuestra que la crisis, de vivienda o de cualquier orden, no se resuelve solo destinando más fondos"), " a un área específica. Hay que tener planes de largo plazo, y sobre todo, una discusión con la altura de miras requerida, que permita atacar las raíces del problema.")
            )
        )
    )
  )
...
tab_eje2 <- bslib::nav_panel(
  ...
  tab_wrap(
    ...
    p(class = "sec-desc", "¿Cuál es el costo de vivir en un campamento? ¿Por qué es tan difícil salir de ahí?."),
    ...
    div(class = "dim-panel-area", id = "e2_panel_area",
        ...
        div(id = "e2_panel_1", class = "dim-panel",
            ...
            div(id = "ep_e2d1", class = "exp-panel",
                p(class = "exp-p", "Vivir en un campamento no es el resultado de no trabajar. ", tags$strong("El 84,9% de los habitantes de campamentos tiene ocupación"), ". La brecha no está en la disposición a trabajar, sino en la calidad del empleo al que pueden acceder."),
                p(class = "exp-p", "Casi la mitad, un ", tags$strong("49,9%"), ", lo hace sin contrato formal, casi el doble del promedio nacional (26,8%), que además incluye a los propios habitantes de campamentos en su cálculo, lo que hace la brecha real aún mayor. Sin contrato no hay cotizaciones, licencia médica ni ningún tipo de red de protección si algo falla."),
                p(class = "exp-p", "Esa informalidad tiene un costo medible: los trabajadores informales ganan en promedio 11% menos que los formales con tareas comparables. Para la población migrante, sobrerrepresentada en campamentos, la brecha es mucho más severa: $508.168 mensuales frente a $824.036 de los trabajadores formales, ", tags$strong("una diferencia del 38%"), "."),
                p(class = "exp-p", "El problema no termina en el sueldo del mes. Para acceder a un subsidio habitacional se exige ahorro acreditado, y para ahorrar se necesita ingreso estable y formal. ", tags$strong("La informalidad es la primera barrera de un sistema que exige, como puerta de entrada, lo que vivir en el campamento impide construir"), ".")
            )
        ),
        ...
        div(id = "e2_panel_2", class = "dim-panel",
            ...
            div(id = "ep_e2d2", class = "exp-panel",
                ...
                p(class = "exp-p", "En la Región Metropolitana, la distancia desde algunos campamentos hasta el centro de salud primaria más cercano supera los ", tags$strong("3,8 kilómetros"), ". No existe un estudio nacional equivalente con esta metodología, así que no sabemos qué pasa en regiones más alejadas. No hay motivos para pensar que la situación sea mejor."),
                p(class = "exp-p", "La consecuencia más concreta de esa distancia es que, cuando se necesita ayuda urgente, esa ayuda no siempre llega: ", tags$strong("18,75% de los hogares en campamentos"), " declara que la ambulancia no llega físicamente donde viven. No es un problema de cobertura en el papel, sino que el sistema de emergencias no reconoce esas direcciones."),
                p(class = "exp-p", "Y la necesidad de esa atención no es menor: ", tags$strong("el 91,3% de los campamentos del país está expuesto a al menos una amenaza física"), ", ya sea remoción en masa, incendio, inundación o riesgo de origen humano, de las 889 que el MINVU ha identificado. El mismo territorio que concentra más riesgo es el que tiene menos capacidad de respuesta cuando ese riesgo se materializa.")
            )
        ),
        ...
        div(id = "e2_panel_3", class = "dim-panel",
            ...
            div(id = "ep_e2d3", class = "exp-panel",
                p(class = "exp-p", "Cuatro de cada diez personas que viven en un campamento son menores de edad. La mayoría, ", tags$strong("un 57%, son chilenos"), ". El rezago educativo no es un problema de origen migratorio, es un problema de entorno."),
                p(class = "exp-p", "Esos niños tienen ", tags$strong("15% de probabilidad de presentar rezago escolar"), ", frente a un 1,7% a nivel nacional: casi nueve veces más. Es la brecha relativa más pronunciada de todo este eje. Un 4% no asiste a ningún establecimiento educacional, aunque esa cifra puede incluir niños migrantes en proceso de regularización documental, no solo ausencia por decisión familiar."),
                p(class = "exp-p", "No es que estas familias no valoren la educación. Lo que sucede es que ", tags$strong("el entorno produce rezago incluso cuando sí la valoran"), ". Sin dirección formal, sin estabilidad residencial, sin certeza de cuánto tiempo más van a vivir ahí, sostener una trayectoria escolar continua es más difícil incluso para quien lo intenta con todas sus fuerzas."),
                p(class = "exp-p", tags$strong("Un niño que nace en un campamento hereda una crisis que no eligió"), ". En promedio, le esperan más de una década por delante antes de que exista una solución habitacional definitiva.")
            )
        ),
        ...
        div(id = "e2_panel_4", class = "dim-panel",
            ...
            div(id = "ep_e2d4", class = "exp-panel",
                p(class = "exp-p", "Más de la mitad de los hogares en campamentos, ", tags$strong("un 55%, vive en situación de inseguridad alimentaria"), ", frente a un 18% a nivel nacional: tres veces más. La comparación cruza dos años distintos, Recrea 2025 para campamentos y CASEN 2022 para el dato nacional, y esa diferencia metodológica queda declarada, no disuelta en el promedio."),
                p(class = "exp-p", "El origen es el mismo que atraviesa este eje completo: ", tags$strong("el ingreso insuficiente documentado en la Dimensión 1"), ", combinado con un acceso más limitado a mercados formales de alimentos."),
                p(class = "exp-p", "Esa inseguridad llega hasta el colegio: ", tags$strong("24,8% de los niños en campamentos"), " no recibe alimentación en su establecimiento educacional."),
        ...
        div(id = "e2_panel_5", class = "dim-panel",
            ...
            div(id = "ep_e2d5", class = "exp-panel",
                p(class = "exp-p", "Salir de un campamento no depende solo de querer hacerlo. ", tags$strong("El 35% de las familias lleva más de 14 años esperando"), " una solución habitacional. Esto es una generación completa: un niño que nació en el campamento ya es adolescente."),
                p(class = "exp-p", "La espera tampoco avanza al mismo ritmo para todos. ", tags$strong("Solo el 4% de los campamentos catastrados"), " tiene un proyecto habitacional en ejecución. Ese dato proviene de la muestra de TECHO-Chile, que sobrerrepresenta a los campamentos con más capacidad de organización. Si incluso entre los mejor organizados solo 4 de cada 100 tiene un proyecto en marcha, la situación del resto probablemente es peor."),
                p(class = "exp-p", "Para los hogares migrantes existe además una barrera de diseño. El sistema exige ahorro previo acreditado y, para acceder a la residencia definitiva, dos años de residencia temporal previa. Ambas condiciones son difíciles de cumplir para quienes llevan poco tiempo en Chile, como muestra la Dimensión 1. ", tags$strong("El requisito no distingue entre quien puede cumplirlo y quien no"), ". El resultado es que excluye, en la práctica, a quienes más lo necesitan."),
                p(class = "exp-p", "Cuando estas barreras terminan en desalojo el problema solo se traslada. Entre 2022 y 2023, ", tags$strong("al menos 1.710 familias fueron desalojadas"), " sin alternativa habitacional adecuada, y terminaron en otro campamento, de allegadas o en la calle."),
                p(class = "exp-p", "El ahorro mínimo exigido no es una cifra enorme. ", tags$strong("El problema es sostenerlo con un ingreso que parte más bajo solo por ser informal"), "."),
                div(class = "sim-ahorro",
                    ...
                    div(class = "sim-control",
                        tags$label("Ingreso mensual de referencia", `for` = "sim_ingreso"),
                    ...
                    div(class = "sim-control",
                        tags$label("% que se ahorra", `for` = "sim_pct"),
                    ...
                    div(class = "sim-comparacion",
                        div(class = "sim-columna",
                            p(class = "sim-col-label", "Con trabajo formal"),
                            p(class = "sim-card-val", id = "sim_tiempo_formal", "8 meses"),
                            p(class = "sim-col-sub", "mismo ingreso, sin descuento")
                        ),
                        div(class = "sim-columna sim-columna-acento",
                            p(class = "sim-col-label", "Con la brecha de informalidad"),
                            p(class = "sim-card-val sim-card-val-acento", id = "sim_tiempo_informal", "9 meses"),
                            p(class = "sim-col-sub", "11% menos ingreso, mismo % de ahorro")
                        )
                    ),
                    p(class = "sim-nota", "El 11% es el diferencial salarial medido entre trabajadores informales y formales con tareas comparables (CLAPES UC, 2024). El cálculo asume, además, que el ahorro nunca se interrumpe. Eso es lo más difícil de sostener cuando casi la mitad de quienes viven en campamento no tiene contrato:"),
                    tags$ul(class = "sim-barreras",
                            tags$li("49,9% no tiene contrato de trabajo, frente a 26,8% a nivel nacional"),
...
tab_qss <- bslib::nav_panel(
  ...
  tab_wrap(
    ...
    div(class = "eje-cierre",
        p(class = "eje-cierre-label", "Reflexiones finales"),
        p(class = "eje-cierre-txt", HTML("Ninguno de los cuatro casos anteriores resolvió su déficit eliminando asentamientos informales. Lo resolvieron interviniendo el mercado de vivienda o de suelo de forma sostenida. Atacando la raíz y no la rama. Chile no ha tomado ese camino. Entre 2022 y 2023, 1.710 familias fueron desalojadas de campamentos, en su mayoría sin alternativa habitacional adecuada. Terminaron en otro campamento, en situación de allegamiento o en la calle. Si queremos, como país, alcanzar soluciones suficientes, debemos tener una discusión con altura de miras sobre el problema habitacional que enfrentamos. Porque pasar una retroexcavadora por todos los campamentos de Chile no va a impedir que <strong>el 80% de las familias chilenas no pueda comprar una vivienda</strong>.")),
  ...
  tags$script(HTML("
    ...
    (function() {
      ...
      function fmtMeses(meses) {
        ...
        var anios = meses / 12;
        return (Math.round(anios * 10) / 10).toFixed(1) + ' años';
      }
      ...
      function renderSim() {
        ...
        document.getElementById('sim_ingreso_out').textContent = '$' + ingreso.toLocaleString('es-CL');
        document.getElementById('sim_pct_out').textContent = pct + '%';

        var ahorroFormal = ingreso * (pct / 100);
        var ahorroInformal = (ingreso * (1 - BRECHA_INFORMAL)) * (pct / 100);

        document.getElementById('sim_tiempo_formal').textContent = fmtMeses(AHORRO_MINIMO / ahorroFormal);
        document.getElementById('sim_tiempo_informal').textContent = fmtMeses(AHORRO_MINIMO / ahorroInformal);
      }
...
server <- function(input, output, session) {
  ...
  # ── Mapa de campamentos (Antecedentes) ───────────────────────────────────
  # geojson_campamentos_final ya viene con estilo y popup inyectados (armado
  # una sola vez al arrancar la app, ver sección de datos). El resaltado al
  # pasar el mouse y el binding del popup se hacen con Leaflet.js directo vía
  # onRender, no con argumentos de addGeoJSON (esa función no los acepta).
  output$mapa_campamentos <- renderLeaflet({
    leaflet(options = leafletOptions(minZoom = 4, maxZoom = 9)) %>%
      setView(lng = -71.0, lat = -35.5, zoom = 4) %>%
      addProviderTiles("CartoDB.PositronNoLabels") %>%
      addGeoJSON(
        geojson_campamentos_final,
        weight = 0.4, color = "#FFFFFF"
      ) %>%
      addLegend(
        pal = pal_campamentos, values = c(0, datos_campamentos$hogares), opacity = 0.85,
        title = "Hogares en<br>campamentos", position = "bottomright"
      ) %>%
      onRender("
        function(el, x) {
          var map = this;
          map.eachLayer(function(layer) {
            ...
            var base = layer.feature.properties.style || {};
            layer.on('mouseover', function(e) {
              e.target.setStyle({weight: 1.5, color: '#333333', fillOpacity: 0.95});
              e.target.bringToFront();
            });
            layer.on('mouseout', function(e) {
              e.target.setStyle({
                weight: base.weight || 0.4,
                color: base.color || '#FFFFFF',
                fillOpacity: base.fillOpacity || 0.85
              });
            });
  ...
  outputOptions(output, "mapa_campamentos", suspendWhenHidden = FALSE)
